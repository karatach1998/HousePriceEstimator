...
async def get_restaurants_info(longitude, latitude, width, height):
    ...
    async with get_session(service, browser) as session:
        url = f"https://yandex.ru/maps/213/moscow/search/%D0%93%D0%B4%D0%B5%20%D0%BF%D0%BE%D0%B5%D1%81%D1%82%D1%8C/?from=mapsapi&indoorLevel=1&ll={longitude},{latitude}&sll={longitude},{latitude}&sspn={width},{height}&z=16"
        ...
        for card in await session.get_elements("//div[@class='search-snippet-view__body _type_business']", SelectorType.xpath):
            try:
                ...
                await R.geoadd(f"organization:food", (lon, lat, json.dumps(info)))
...
async def main():
    ...
    while r < 0.8:
        ...
        path.append([lat, lon])
    start = 0
    for lat, lon in islice(tqdm(path[start:], initial=start), None):
